            ...
            {loading && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                ...
                <div style={{ background: 'white', border: '0.5px solid rgba(0,0,0,0.1)', borderRadius: 16, borderBottomLeftRadius: 4, padding: '12px 16px', display: 'flex', gap: 4 }}>
                  {[0,1,2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#ccc', animation: 'blink 1.2s infinite', animationDelay: `${i*0.2}s` }} />)}
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.93)', borderRadius: 20, padding: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: 'rgba(0,0,0,0.05)', borderRadius: 10, padding: 3 }}>
              {Object.entries(tabLabels).map(([key, label]) => (
                <button key={key} onClick={() => setActiveTab(key)} style={{ flex: 1, padding: '7px 4px', borderRadius: 7, border: 'none', background: activeTab === key ? 'white' : 'transparent', color: activeTab === key ? '#7c5cbf' : '#999', fontWeight: activeTab === key ? 700 : 400, fontSize: 11, cursor: 'pointer', fontFamily: 'Noto Sans JP, sans-serif' }}>{label}</button>
              ))}
            </div>
